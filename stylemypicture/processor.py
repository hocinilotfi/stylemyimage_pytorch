# _____________Define the model____________
...
# ____________ Implement utils functions for read/write image _________
...
def get_PIL_from_tensor(tensor, cuda=False):
    if cuda:
        img = tensor.clone().cpu().clamp(0, 255).numpy()
    else:
        img = tensor.clone().clamp(0, 255).numpy()
